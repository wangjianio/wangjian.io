...
if (!isset($_GET['c'])) {
    ...
    if (empty($cate_1)) {
        ...
        $sql = "UPDATE $table SET {$t_type}_4 = ? WHERE {$t_type}_1 = ? AND {$t_type}_2 = ? AND {$t_type}_3 = ? AND {$t_type}_4 = ?";
        if ($stmt = $database->mysqli->prepare($sql)) {
            $stmt->bind_param("sssss", $new_cate_name, $cate_1, $cate_2, $cate_3, $cate_4);
        }
    } else {
        $sql = "UPDATE $table SET {$t_type}_5 = ? WHERE {$t_type}_1 = ? AND {$t_type}_2 = ? AND {$t_type}_3 = ? AND {$t_type}_4 = ? AND {$t_type}_5 = ?";
        if ($stmt = $database->mysqli->prepare($sql)) {
            $stmt->bind_param("ssssss", $new_cate_name, $cate_1, $cate_2, $cate_3, $cate_4, $cate_5);
        }
    }
